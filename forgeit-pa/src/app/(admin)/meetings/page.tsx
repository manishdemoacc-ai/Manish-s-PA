  ...
  function MeetingCard({ meeting }: { meeting: Meeting }) {
    return (
      <div className="bg-card border border-border rounded-xl p-4 space-y-3">
        ...
        {meeting.preferred_dates &&
          meeting.preferred_dates.length > 0 && (
            <div className="text-xs text-muted-foreground">
              <span className="font-medium text-foreground">
                Preferred:
              </span>{' '}
              {meeting.preferred_dates.join(', ')}
            </div>
          )}
  ...
  return (
    <div className="flex gap-2">
      ...
      >
        <button
          type="submit"
          className="px-3 py-1.5 bg-green-600 text-white rounded-lg text-xs font-medium hover:opacity-90 transition-opacity"
        >
          Confirm
        </button>
      </form>
